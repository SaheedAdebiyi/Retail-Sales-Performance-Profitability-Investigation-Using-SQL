...
FROM retail_sales;
  ...
  -- Sales by time period of the day and respective category
 SELECT category, IF(HOUR(sale_time) BETWEEN 6 AND 11, 'Morning',
       IF(HOUR(sale_time) BETWEEN 12 AND 17, 'Afternoon',
		IF(HOUR(sale_time) BETWEEN 18 AND 21, 'Evening', 'Night'))) AS time_period,
    COUNT(*) AS transactions,
    ROUND(SUM(total_sale)) AS revenue,
    ROUND(SUM(total_sale - total_cost)) AS profit
FROM retail_sales
GROUP BY time_period, category
ORDER BY MIN(HOUR(sale_time));

 -- Sales by time period of the day
 SELECT  IF(HOUR(sale_time) BETWEEN 6 AND 11, 'Morning',
       IF(HOUR(sale_time) BETWEEN 12 AND 17, 'Afternoon',
		IF(HOUR(sale_time) BETWEEN 18 AND 21, 'Evening', 'Night'))) AS time_period,
    COUNT(*) AS transactions,
    ROUND(SUM(total_sale)) AS revenue,
